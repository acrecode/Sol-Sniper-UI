...
/*
  SolSniper — Control Panel (reference UI)
  Single-file React component. Wire to the Fastify API described in instructions.md:
    PUT  /config            -> save the active strategy
    GET  /ws                -> stream detections / decisions / fills / exits
    POST /control/{start|stop|kill|flush}
  All data here is mock/placeholder so the surface is fully visible without a backend.
*/
...
const DEFAULTS = {
  ...
  copyTrade: "",
  reqMintRevoked: true,
  reqFreezeRevoked: true,
  reqUpdateRevoked: false,
  tokenProgram: "spl_only",
  reqLpBurned: true,
  maxDevPct: 5,
  maxTop10Pct: 30,
  maxSinglePct: 15,
...
const PRESETS = {
  Conservative: {
    sources: ["pumpswap_pool"], phases: ["post_grad"], curveMin: 0, curveMax: 100,
    reqMintRevoked: true, reqFreezeRevoked: true, reqUpdateRevoked: true, tokenProgram: "spl_only",
    reqLpBurned: true, maxDevPct: 3, maxTop10Pct: 22, maxSinglePct: 10, maxBundlePct: 12,
    minHolders: 60, minLiqUsd: 15000, reqHoneypotSim: true, maxBuyTax: 0, maxSellTax: 0, maxRugScore: 20,
    solPerToken: 0.1, maxSlippage: 6, feeMode: "auto", feeAggr: 1, jito: false, maxConcurrent: 3, dailyCapSol: 1,
    tpLadder: "120:100", stopLoss: 25, trailing: 0, maxHoldMin: 45, liqDropExit: 30, breakeven: true,
    aggrTP: false, aggrTrigger: 80, aggrSellPct: 50, fastExit: false,
  },
  Balanced: { ...DEFAULTS },
  Degen: {
    sources: ["pumpfun_curve", "pumpswap_pool"], phases: ["new_mint", "mid_curve", "at_grad"],
    curveMin: 0, curveMax: 100, reqMintRevoked: true, reqFreezeRevoked: true, reqUpdateRevoked: false,
    tokenProgram: "allow_t22", reqLpBurned: false, maxDevPct: 12, maxTop10Pct: 45, maxSinglePct: 22,
    maxBundlePct: 30, minHolders: 5, minLiqUsd: 0, reqHoneypotSim: true, maxBuyTax: 5, maxSellTax: 5,
    maxRugScore: 60, solPerToken: 0.5, maxSlippage: 25, feeMode: "auto", feeAggr: 4, jito: true,
    maxConcurrent: 8, dailyCapSol: 4, tpLadder: "100:50, 400:25, 1000:15", stopLoss: 40, trailing: 20,
    maxHoldMin: 20, liqDropExit: 50, breakeven: true,
    aggrTP: true, aggrTrigger: 25, aggrSellPct: 90, fastExit: true,
  },
  "Graduation snipe": {
    sources: ["pumpswap_pool"], phases: ["at_grad"], curveMin: 90, curveMax: 100,
    reqMintRevoked: true, reqFreezeRevoked: true, reqUpdateRevoked: false, tokenProgram: "spl_only",
    reqLpBurned: true, maxDevPct: 8, maxTop10Pct: 35, maxSinglePct: 18, maxBundlePct: 25, minHolders: 15,
    minLiqUsd: 9000, reqHoneypotSim: true, maxBuyTax: 0, maxSellTax: 0, maxRugScore: 40, solPerToken: 0.3,
    maxSlippage: 15, feeMode: "auto", feeAggr: 3, jito: true, maxConcurrent: 5, dailyCapSol: 3,
    tpLadder: "80:60, 300:25", stopLoss: 30, trailing: 18, maxHoldMin: 15, liqDropExit: 45, breakeven: true,
    aggrTP: true, aggrTrigger: 35, aggrSellPct: 85, fastExit: true,
  },
};

const FEED = [
  { t: "12:04:51", mint: "9xQe…pump", src: "PumpSwap", act: "BUY", reason: "passed · rug 18 · dev 2.1% · liq $14k", good: true, size: "0.25◎" },
  { t: "12:04:48", mint: "Hn3k…bonk", src: "pump.fun", act: "SKIP", reason: "bundle 41% > max 20%", good: false },
  { t: "12:04:39", mint: "4Tay…fwap", src: "PumpSwap", act: "SKIP", reason: "honeypot sim failed (sell reverted)", good: false },
  { t: "12:04:31", mint: "Bv8m…rayd", src: "Raydium", act: "SKIP", reason: "top10 52% > max 30%", good: false },
  { t: "12:04:22", mint: "2Kpl…pump", src: "PumpSwap", act: "BUY", reason: "passed · rug 24 · copy-trade hit", good: true, size: "0.25◎" },
  { t: "12:04:08", mint: "Qz9d…pump", src: "pump.fun", act: "SKIP", reason: "freeze authority not revoked", good: false },
];

const POSITIONS = [
  { mint: "9xQe…pump", entry: "0.0000042", price: "0.0000089", pnl: 112, size: "0.25◎", exit: "TP 100:50 · SL -35%" },
  { mint: "2Kpl…pump", entry: "0.0000118", price: "0.0000131", pnl: 11, size: "0.25◎", exit: "trail 20% · BE armed" },
  { mint: "Lm44…fwap", entry: "0.0000301", price: "0.0000204", pnl: -32, size: "0.30◎", exit: "SL -35% (near)" },
];
...
export default function SolSniper() {
  ...
  const botReady = (wTab === "key" && keyInput.trim().length > 40) || (wTab === "phantom" && phantom);
  const botPub = wTab === "phantom" ? "Phntm…9xQk" : (keyInput.trim() ? "Bot7…k2Va" : null);

  const set = (k, v) => setCfg((c) => ({ ...c, [k]: v }));
  const applyPreset = (name) => { setPreset(name); setCfg({ ...DEFAULTS, ...PRESETS[name] }); };
  const toggleArr = (k, id) => set(k, cfg[k].includes(id) ? cfg[k].filter((x) => x !== id) : [...cfg[k], id]);

  const capUsed = 0.75, capPct = Math.min(100, (capUsed / cfg.dailyCapSol) * 100);
  const wlCount = useMemo(() => cfg.whitelist.split(/\s+/).filter(Boolean).length, [cfg.whitelist]);
  const ctCount = useMemo(() => cfg.copyTrade.split(/\s+/).filter(Boolean).length, [cfg.copyTrade]);
  ...
  return (
    <div className="mn" style={{ background: C.bg, color: C.text, minHeight: "100vh", fontSize: 13 }}>
      ...
      <div style={{ position: "sticky", top: 0, zIndex: 10, background: C.panel, borderBottom: `1px solid ${C.border}`,
        ...
        <Pill label="STREAM" value="gRPC ●" valColor={C.green} />
        <Pill label="GATES" value="OK" valColor={C.green} />

        <div style={{ display: "flex", flexDirection: "column", gap: 3, minWidth: 150 }}>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: C.muted }}>
            <span>DAILY CAP</span><span>{capUsed}◎ / {cfg.dailyCapSol}◎</span>
          </div>
        ...
        <div style={{ display: "flex", border: `1px solid ${C.border}`, borderRadius: 6, overflow: "hidden" }}>
          {["dry-run", "devnet", "live"].map((m) => (
            <button key={m} onClick={() => setMode(m)} style={{
              ...
              color: mode === m ? "#fff" : C.muted, border: "none", padding: "7px 12px", cursor: "pointer",
              fontSize: 11, textTransform: "uppercase" }}>{m}</button>
          ))}
        </div>

        <button onClick={() => botReady && setRunning((r) => !r)} disabled={!botReady}
          title={botReady ? "" : "Load the bot wallet first"}
          style={{ ...btn(running ? C.panel2 : (botReady ? C.accent : C.panel2), running ? C.text : (botReady ? "#160c03" : C.faint)),
            opacity: botReady ? 1 : 0.5, cursor: botReady ? "pointer" : "not-allowed" }}>
          {running ? "■ STOP" : "▶ ARM"}
        </button>
        <button onClick={() => setRunning(false)} style={btn("transparent", C.red, C.red)}>⏻ KILL</button>
        <button style={btn("transparent", C.muted, C.border)}>⇩ FLUSH</button>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "minmax(380px, 1fr) minmax(420px, 1.15fr)", gap: 16, padding: 16, alignItems: "start" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <Panel>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
              <H>Wallet</H>
              {botReady
                ? <span style={{ fontSize: 11, color: C.green }}>● {botPub}&nbsp;·&nbsp;3.42◎</span>
            ...
            {wTab === "key" ? (
              <>
                ...
                <div style={{ fontSize: 10, color: C.faint, marginTop: 7, lineHeight: 1.5 }}>
                  ...
                  with what you'll deploy — never your main wallet. Key stays server-side, never logged.
                </div>
              </>
            ) : (
              <>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16, position: "sticky", top: 80 }}>
          <Panel>
            ...
            <div className="scroll" style={{ maxHeight: 260, overflowY: "auto", display: "flex", flexDirection: "column", gap: 1 }}>
              {FEED.map((f, i) => (
                <div key={i} className={i === 0 ? "live-row" : ""} style={{ display: "flex", alignItems: "center", gap: 10,
                  padding: "8px 6px", borderBottom: `1px solid ${C.panel2}` }}>
                  <span style={{ color: C.faint, fontSize: 11 }}>{f.t}</span>
                  <span style={{
                    fontSize: 10, fontWeight: 600, padding: "2px 7px", borderRadius: 4,
                    color: f.good ? C.green : C.muted, background: f.good ? "rgba(95,211,138,.1)" : C.panel2,
                    border: `1px solid ${f.good ? "rgba(95,211,138,.3)" : C.border}` }}>{f.act}</span>
                  <span style={{ color: C.text, fontSize: 12 }}>{f.mint}</span>
                  <span style={{ color: C.faint, fontSize: 10 }}>{f.src}</span>
                  <span style={{ flex: 1 }} />
                  <span style={{ color: f.good ? C.text : C.muted, fontSize: 11, textAlign: "right" }}>
                    {f.size ? <b style={{ color: C.accent }}>{f.size} · </b> : null}{f.reason}
                  </span>
                </div>
              ))}
            </div>
          ...
          <Panel>
            ...
            {POSITIONS.map((p, i) => (
              <div key={i} style={{ padding: "10px 6px", borderBottom: `1px solid ${C.panel2}` }}>
                <div style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr .7fr .8fr", alignItems: "center" }}>
                  <span style={{ color: C.text }}>{p.mint}</span>
                  <span style={{ color: C.muted, fontSize: 11 }}>{p.entry}→{p.price}</span>
                  <span style={{ color: C.muted }}>{p.size}</span>
                  <span style={{ textAlign: "right", fontWeight: 600, color: p.pnl >= 0 ? C.green : C.red }}>
                    {p.pnl >= 0 ? "+" : ""}{p.pnl}%
                  </span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 6 }}>
                  <span style={{ color: C.faint, fontSize: 10 }}>{p.exit}</span>
                  <button style={{ background: "transparent", border: `1px solid ${C.border}`, color: C.muted,
          ...
          <div style={{ fontSize: 10, color: C.faint, lineHeight: 1.6, padding: "0 4px" }}>
            Reference UI · mock data. Mode <b style={{ color: mode === "live" ? C.red : C.accent }}>{mode}</b>.
...
/* ───────────── primitives ───────────── */
const inp = {
  width: "100%", background: "#0c0a09", border: `1px solid ${C.border}`, color: C.text,
  borderRadius: 6, padding: "8px 10px", fontSize: 13,
};
const btn = (bg, color, border) => ({
  background: bg, color, border: `1px solid ${border || bg}`, borderRadius: 6,
  padding: "8px 14px", cursor: "pointer", fontSize: 12, fontWeight: 600, whiteSpace: "nowrap",
});

function Panel({ children }) {
  return <div style={{ background: C.panel, border: `1px solid ${C.border}`, borderRadius: 10, padding: 16 }}>{children}</div>;
}
function H({ children }) {
  return <span className="bn" style={{ fontSize: 21, color: C.text }}>{children}</span>;
}
...
function Label({ children, hot }) {
  return <div style={{ fontSize: 11, color: hot ? C.accent : C.muted, marginTop: 10, marginBottom: 5 }}>{children}</div>;
}
...
function Num({ label, v, on, step = 1 }) {
  return (
    <div style={{ flex: 1 }}>
      <Label>{label}</Label>
      <input type="number" step={step} value={v} onChange={(e) => on(+e.target.value)} style={inp} />
    </div>
  );
}
...
function Select({ v, on, opts }) {
  return (
    <select value={v} onChange={(e) => on(e.target.value)} style={{ ...inp, cursor: "pointer" }}>
